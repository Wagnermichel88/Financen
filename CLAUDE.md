# WagnerInvest – Family Office Gremium

Projektkontext für Claude Code. Übernommen aus einem claude.ai‑Chat (Stand 24.09.2026).
Der vollständige ältere Verlauf liegt in `verlauf/gespraechsverlauf_bis_22-09-2026.txt` (nur bei Bedarf gezielt durchsuchen, nicht komplett lesen – die Datei ist gross).

## Nutzer
- Michel Wagner, Lyss (Schweiz), geb. 16.10.1988
- 100 % bei Bernmobil, Schichtdienst mit Zulagen
- Nettoauszahlung 2026 schwankt ca. 5550–6490 CHF/Monat (Budget rechnet mit 5650), plus 13. Monatslohn (im Budget noch nicht verplant)
- Zahlt Alimente 950 CHF/Monat
- Budgetplan gemeinsam mit Partnerin Nadja Joder; Haushaltsüberschuss ca. 171 CHF/Monat (Michel ca. 164, Nadja ca. 7). Überschrift „Bruttolohn“ im Budget ist falsch beschriftet, gemeint ist netto.

## Kommunikation – verbindlich
- Immer komplett Deutsch, auch Zahlen, damit die Sprachwiedergabe alles deutsch vorliest: deutsches Zahlenformat (1,5), Einheiten ausschreiben (Dollar, Franken, Prozent, Milliarden), keine Pfeile/Malzeichen, deutsche Fachbegriffe (freier Mittelzufluss, Auftragsbestand, Prognose, erwartetes Kurs‑Gewinn‑Verhältnis, Bewertungspunkte, Klasse A/B), möglichst wenige Tabellen.
- Direktes, ehrliches Feedback. Keine automatische Zustimmung, keine Schmeicheleien. Schwächen, Selbsttäuschung, Vermeidung und Opportunitätskosten aktiv benennen.
- Meetings als Gremium: Jedes Mitglied spricht charakterspezifisch und menschlich, argumentiert, widerspricht, hinterfragt. Meinungsverschiedenheiten austragen, nicht glätten.
- Fiktive Berater, keine realen Personen. Keine erfundenen Zitate realer Personen.
- Claude kann keine Trades ausführen; der Nutzer führt Aufträge selbst aus und schickt Bildschirmfotos.
- Kein Disclaimer-Satz in Antworten und Protokollen (Wunsch des Nutzers, 25.09.).

## Gremium (fiktiv)
- Julian Vetsch – CIO, Vorsitz, strukturiert, pragmatisch
- Diego Álvarez – Beteiligungen, renditeorientiert, Gegenstimme gegen zu viel Vorsicht
- Malik Owusu – Immobilien (inaktiv)
- Dr. Sophia Keller – Risiko, Limiten, Klumpenrisiken, hartnäckig
- Matteo Rossi – Recherche/Scout: Beobachtungsliste, Bewertungsraster, Depot‑Status (prüft bei jedem Update des Nutzers)
- Prof. Mila Novak – Verhaltensökonomin, stellt unbequeme Fragen (z. B. „Würden Sie heute neu kaufen?“)
- Noah Brandt – Steuerjurist, Protokoll, Fristen, Säule 3a
- Elena Marchetti – Nachfolge (inaktiv); Felix Berger – Governance (inaktiv)
- Dr. Mia Lindqvist – Makro/Zinsen; Theo Kastner – Nachrichten/Lagebild
- Nora Lindberg – Medien (inaktiv)

Tägliches Meeting mit acht Traktanden: Nachrichten, Wirtschaftsdaten, Portfolio‑Status, Beobachtungsliste, Risiko‑Check, offene Aufgaben, Verhaltens‑Check, nächster Schritt.

## Regelprotokoll
1. Positionslimite: max. 10 % Depotwert pro Einzeltitel
2. Länderlimite: max. 20 % pro Land ausserhalb Heimmarkt
3. Kalendertrigger: Überprüfung alle 3 Monate
4. Bargeldband 5–30 %: bis 30 % nur im Bärenmarkt (Leitindex mind. 20 % unter Allzeithoch); sonst Rückführung Richtung 5–10 %, schrittweise mit dokumentiertem Stufenplan
5. Diversifikation: Säule 3a = breite Streuung; eToro = aktiver Einzelwert‑Satellit, kein ETF
6. Krypto‑Zielquote 5 % des Gesamtvermögens, Umsetzung erst nach Sperrfristende (ca. Sept. 2027), kein weiterer Ausbau
7. 12‑Monats‑Trigger: Neubeurteilung ETH/SOL/TRX und FLOKI
8. Ausnahme 20 %-Limite: nur Bewertungspunkte ≥ 85, schriftliche These, Gegenzeichnung 2 Mitglieder, quartalsweise
9. Präzedenzfall BYD: Ausnahme nicht anwendbar
10. Bewertungsfaktor: kein Scoring‑Kriterium, wirkt auf Zielgrösse. Erwartetes KGV gegen Branchenmedian (Quelle einheitlich GuruFocus). Über dem doppelten Median = halbe Zielgrösse, gestaffelt. Bei Verlust nicht anwendbar = kein Zukauf. Vor jedem Kauf ein Satz zur eingepreisten Erwartung. Gilt nur für künftige Käufe (Bestand vorübergehend ausgenommen). PEG am 22.09. verworfen.
11. Themenobergrenze (24.09.): max. 40 % pro Anlagethema (hart); über 30 % kein neues Geld ins Thema, kein Zwangsverkauf. Thema KI = ASML, Broadcom, CrowdStrike, Palantir, IonQ, GE Vernova, Alphabet.
12. Monatliche Einzahlung: 200 CHF am 25. jedes Monats aufs eToro‑Konto; innert 5 Handelstagen investieren, in bestehende Positionen am weitesten unter Zielgrösse; neue Titel nur per Meeting‑Beschluss; Bewertungsregel gilt; Umrechnungsgebühr CHF→USD prüfen.
13. Entscheidungsfrage bei durchgefallenen Bestandstiteln: „Würde ich heute neu kaufen?“ Nein = verkaufen.
14. Ergänzung zu 12 (24.09.): Ist keine bestehende Position zukaufsfähig, bleibt die Einzahlung Bargeld bis zum nächsten Beschluss über einen neuen Titel; im Protokoll vermerken.

Offene Traktanden: Länderlimite 20 % gegen USA‑Anteil ca. 59 % (neu fassen oder bewusst ausnehmen); Vergleichsgruppen‑Regel präzisieren (Probleme bei GE Vernova, Visa); zweites Raster für Banken/Versicherer/Basiskonsum oder bewusster Verzicht.

## Bewertungsraster (100 Punkte, 4 × 25)
1. Finanzen: Umsatzwachstum > 15 %, freier Mittelzufluss, Bilanz
2. Momentum: Auftragsbestand/Jahresumsatz > 1,5, Visibilität, Prognose angehoben
3. Markt: Sektordynamik, relative Stärke, Marktanteil
4. Skalierung: Margen, Marktgrösse, Burggraben
Klasse A 85–100 (Zielgrösse meist 6–8 %), Klasse B 65–84 (Zielgrösse 4 %), unter 65 durchgefallen.
Bekannte Schwäche: Das Raster bevorzugt Wachstum/Tech und sortiert defensive Qualität aus – mitverantwortlich für den KI‑Klumpen.

## Vermögen (Stand 25.09.2026)
- eToro (USD‑Konto): Kontowert 1918 Dollar, Bargeld 575,76 Dollar = 30,0 % (BYD verkauft; Einzahlung 25.09. noch nicht eingegangen)
- Krypto Bitpanda ca. 722 CHF (ETH, SOL, TRX im Earn gesperrt bis ca. Sept. 2027; FLOKI frei)
- Säule 3a finpension Global 100, ca. 187 CHF; Übertrag von PostFinance (ca. 339 CHF) beantragt 18.09., Frist ca. 9. Oktober
- Sparquote 400 CHF/Monat: 200 eToro, 200 Säule 3a

## Depot‑Status (25.09.2026, Anteil am Kontowert)
Lockheed Martin 8,05 % (A 90, Ziel 8) halten · SpaceX 7,91 % (B 80) halten, kein Zukauf · ASML 7,32 % (A 85, Ziel 8) halten, Themenobergrenze · Alphabet 7,06 % (A 90, Ziel 8) halten, Themenobergrenze · Uber 7,04 % (A 85, Ziel 7) halten · CrowdStrike 5,87 % (A 95) halten, kein Zukauf · GE Vernova 5,44 % (A 95) halten, kein Zukauf · Broadcom 5,28 % (A 85, Ziel 6) halten, Themenobergrenze · ABB 4,04 % (B 80) · Eli Lilly 4,03 % (B 75) · Cheniere 3,65 % (B 80, Ziel 4) · IonQ 2,33 % (B 65) kein Zukauf · Palantir 1,99 % (A 90, Ziel 2 wegen Bewertung).
Thema KI 35,3 %. USA‑Anteil ca. 58,7 % (Länderlimite 20 % verletzt, offenes Traktandum).
Verkauft in dieser Woche: BYD (bestätigt 25.09.), Alibaba, Denali, Klarna (Wiedereinstieg nur bei angehobener Prognose UND positivem operativem Cashflow).

## Beobachtungsliste (Auszug)
Klasse A: Arista 90, Caterpillar 90. Klasse B: Intuitive Surgical 83 (KGV 1,7‑ bis 1,9‑fach Median, neu prüfen), Axon 80 (sehr teuer), Amazon 80, NVIDIA 80, First Solar 78, Microsoft 75, RTX 75, Visa 75 (halbe Zielgrösse, Vergleichsgruppe fraglich), Nextracker 75 (Daten veraltet).
Durchgefallen: Meta 55 (Neuprüfung nach Q3 am 28.10.), JPMorgan 60, Freeport 58, Zurich 55, Costco 53, Novartis 45, Sunrun 45, Viking 45 u. a.
Vollständig im Faktenblatt.

## Offene Aufgaben
- Matteo: Caterpillar (fwd KGV und KI‑Zuordnung) und RTX bis 29.09. prüfen, Kandidaten für die Einzahlung vom 25.09.
- Notreserve festlegen, bevor der 13. Monatslohn verplant wird
- Budget klären: Überschuss 164 CHF gegenüber Sparquote 400 CHF
- eToro‑Verbindung neu anmelden
- Begünstigung Säule 3a zugunsten Nadja bei finpension prüfen (unverheiratet, gesetzlich nicht erbberechtigt)
- Quartalsweises Budgetgespräch mit Nadja
- Matteo: RTX, First Solar, Intuitive Surgical auf Bewertungsregel prüfen (bis 25.10.)
- Bargeld‑Stufenplan: Stufe 2 bis 25.10. (geprüfte Titel ausserhalb KI), Stufe 3 nach 15.11. Richtung 5–10 %
- Sätze zur eingepreisten Erwartung für ASML, Alphabet, Broadcom, Cheniere, Lockheed, Uber bestätigen lassen (ABB, Lilly gelten als angenommen)
- 13. Monatslohn (ca. 5500 CHF netto, Dezember) verplanen
- Krypto‑Reduktion auf 5 % ab Sept. 2027

## Protokolle
- Meetingprotokolle unter `protokolle/JJJJ-MM-TT_meeting.md`

## Faktenblatt
- Jede Entscheidung automatisch ins Faktenblatt übernehmen, aktuelles Portfolio immer aktuell halten.
- Erstellen: `pip install -r requirements.txt` (einmalig), dann `python3 build_factsheet.py` → `out/Family_Office_Gremium_Factsheet.pdf`
- Daten stehen als Python‑Listen im Skript (current_holdings, depot_status, watchlist_results, Regelprotokoll).
